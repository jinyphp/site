...
@section('title', isset($item) ? '트렌드글 수정' : '트렌드글 등록')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center my-3">
        <div>
            <h3>{{ isset($item) ? '트렌드글 수정' : '트렌드글 등록' }}</h3>
            <p class="text-muted mb-0">{{ $config['subtitle'] ?? '' }}</p>
        </div>
        <div>
            <a href="{{ route('admin.cms.board.trend') }}" class="btn btn-secondary">
    ...
    <form action="{{ isset($item) ? route('admin.cms.board.trend.update', $item->id) : route('admin.cms.board.trend.store') }}"
          method="POST">
        @csrf
        @if(isset($item))
            @method('PUT')
            ...
            <div class="tab-pane fade show active" id="basic" role="tabpanel">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="mb-3 row">
                            <label class="col-sm-2 col-form-label">Trend</label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control" name="trend"
                                       value="{{ $item->trend ?? '' }}">
                            </div>
                        </div>

                        <div class="mb-3 row">
                            <label class="col-sm-2 col-form-label">Trend ID</label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control" name="trend_id"
                                       value="{{ $item->trend_id ?? '' }}">
                            </div>
                        </div>
                    </div>
        ...
        <div class="d-flex justify-content-end gap-2 mt-3">
            <a href="{{ route('admin.cms.board.trend') }}" class="btn btn-secondary">취소</a>
