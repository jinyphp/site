...
@section('title', '트렌드글 목록')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center my-3">
        <div>
            <h3>{{ $config['title'] ?? '트렌드글 목록' }}</h3>
            <p class="text-muted mb-0">{{ $config['subtitle'] ?? '' }}</p>
        </div>
        <div>
            <a href="{{ route('admin.cms.board.trend.create') }}" class="btn btn-primary">
                    ...
                    <tbody>
                        @forelse($rows as $item)
                            <tr>
                                ...
                                <td>{{$item->trend}}</td>
                                <td width='200'>{{$item->trend_id}}</td>
                                <td width='200'>
                                    <small class="text-muted">{{$item->created_at}}</small>
                                </td>
                                <td width='150'>
                                    <div class="btn-group btn-group-sm" role="group">
                                        <a href="{{ route('admin.cms.board.trend.edit', $item->id) }}"
                                           class="btn btn-outline-primary" title="수정">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <form action="{{ route('admin.cms.board.trend.destroy', $item->id) }}"
                                              method="POST"
