...
class UpdateController extends Controller
{
    ...
    public function __invoke(Request $request, $id)
    {
        ...
        if ($inventory->quantity != $validated['quantity']) {
            $this->recordMovement($id, $inventory->quantity, $validated['quantity'], 'adjustment');
        }
        ...
        return redirect()
            ->route($this->config['redirect_route'])
            ->with('success', '재고 항목이 성공적으로 수정되었습니다.');
    }

    protected function recordMovement($inventoryId, $oldQuantity, $newQuantity, $type = 'adjustment')
    {
        $movement = [
