                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-control @error('type') is-invalid @enderror"
                                    ...
                                    <option value="product" {{ old('type', $testimonial->type) === 'product' ? 'selected' : '' }}>상품</option>
                                    <option value="service" {{ old('type', $testimonial->type) === 'service' ? 'selected' : '' }}>서비스</option>
                                </select>
                                @error('type')
                                ...
                                <select class="form-control @error('item_id') is-invalid @enderror"
                                    ...
                                    @if($testimonial->type === 'product')
                                        @foreach($products as $product)
                                            <option value="{{ $product->id }}" {{ old('item_id', $testimonial->item_id) == $product->id ? 'selected' : '' }}>
                                                {{ $product->title }}
                                            </option>
                                        @endforeach
                                    @elseif($testimonial->type === 'service')
                                        @foreach($services as $service)
                                            <option value="{{ $service->id }}" {{ old('item_id', $testimonial->item_id) == $service->id ? 'selected' : '' }}>
                                                {{ $service->title }}
                                            </option>
                                        @endforeach
                                    @endif
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text"
                                   ...
                                   id="headline"
                                   name="headline"
                                   value="{{ old('headline', $testimonial->headline) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('content') is-invalid @enderror"
                                      ...
                                      name="content"
                                      rows="5"
                                      placeholder="고객의 상세한 후기를 작성해주세요...">{{ old('content', $testimonial->content) }}</textarea>
                            @error('content')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('rating') is-invalid @enderror" id="rating" name="rating">
                                ...
                                <option value="5" {{ old('rating', $testimonial->rating) == '5' ? 'selected' : '' }}>⭐⭐⭐⭐⭐ (5점)</option>
                                <option value="4" {{ old('rating', $testimonial->rating) == '4' ? 'selected' : '' }}>⭐⭐⭐⭐ (4점)</option>
                                <option value="3" {{ old('rating', $testimonial->rating) == '3' ? 'selected' : '' }}>⭐⭐⭐ (3점)</option>
                                <option value="2" {{ old('rating', $testimonial->rating) == '2' ? 'selected' : '' }}>⭐⭐ (2점)</option>
                                <option value="1" {{ old('rating', $testimonial->rating) == '1' ? 'selected' : '' }}>⭐ (1점)</option>
                            </select>
                            @error('rating')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input"
                                       ...
                                       name="enable"
                                       value="1"
                                       {{ old('enable', $testimonial->enable) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input"
                                       ...
                                       name="verified"
                                       value="1"
                                       {{ old('verified', $testimonial->verified) ? 'checked' : '' }}>
...
function updateItemOptions() {
    ...
    const currentItemId = "{{ $testimonial->item_id }}";
    ...
    if (selectedType === 'product') {
        products.forEach(product => {
            ...
            if (product.id == currentItemId) option.selected = true;
        ...
        services.forEach(service => {
            ...
            if (service.id == currentItemId) option.selected = true;
            itemSelect.appendChild(option);
        });
    }
}
