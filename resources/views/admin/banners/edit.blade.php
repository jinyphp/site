                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h1 class="page-header-title">
                                <i class="bi bi-bullhorn me-2"></i>베너 수정
                            </h1>
                            <p class="page-header-subtitle">베너 정보를 수정합니다.</p>
                        </div>
                        <div>
    ...
    <div class="row">
        <div class="col-xl-8 col-lg-10">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.site.banner.update', $banner->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('type') is-invalid @enderror"
                                        ...
                                        <option value="info" {{ old('type', $banner->type) == 'info' ? 'selected' : '' }}>정보 (Info)</option>
                                        <option value="warning" {{ old('type', $banner->type) == 'warning' ? 'selected' : '' }}>경고 (Warning)</option>
                                        <option value="success" {{ old('type', $banner->type) == 'success' ? 'selected' : '' }}>성공 (Success)</option>
                                        <option value="danger" {{ old('type', $banner->type) == 'danger' ? 'selected' : '' }}>위험 (Danger)</option>
                                        <option value="primary" {{ old('type', $banner->type) == 'primary' ? 'selected' : '' }}>주요 (Primary)</option>
                                        <option value="secondary" {{ old('type', $banner->type) == 'secondary' ? 'selected' : '' }}>보조 (Secondary)</option>
                                    </select>
                                    @error('type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('message') is-invalid @enderror"
                                      ...
                                      required>{{ old('message', $banner->message) }}</textarea>
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <input type="color"
                                           ...
                                           value="{{ old('background_color', $banner->background_color ?: '#007bff') }}">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <input type="number"
                                           ...
                                           id="cookie_days"
                                           name="cookie_days"
                                           value="{{ old('cookie_days', $banner->cookie_days) }}"
                                           min="1"
                                           max="365"
                                           required>
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <div class="form-check form-switch mt-4">
                                        <input class="form-check-input" type="checkbox" id="enable" name="enable" value="1" {{ old('enable', $banner->enable) ? 'checked' : '' }}>
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <div class="form-check form-switch mt-4">
                                        <input class="form-check-input" type="checkbox" id="is_closable" name="is_closable" value="1" {{ old('is_closable', $banner->is_closable) ? 'checked' : '' }}>
                        ...
                        <div class="border-top pt-3 mt-4">
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="submit" class="btn btn-primary">
                                    <i class="fe fe-save me-2"></i>베너 수정
                                </button>
                            </div>
                        </div>
                    </form>
        ...
        <div class="col-xl-4 col-lg-4">
            <div class="card">
                ...
                <div class="card-body">
                    <div id="banner-preview" class="alert alert-{{ $banner->type }}" role="alert">
                        <div class="d-flex align-items-center">
                            <i id="preview-icon" class="{{ $banner->icon ?: 'bi bi-info-circle' }} me-2"></i>
                            <div class="flex-grow-1">
                                <strong id="preview-title">{{ $banner->title }}</strong>
                                <div id="preview-message" class="mt-1">{{ $banner->message }}</div>
                                <div id="preview-link" class="mt-2" style="{{ $banner->link_url ? '' : 'display: none;' }}">
                                    <a href="#" class="btn btn-sm btn-outline-primary">{{ $banner->link_text ?: '링크 텍스트' }}</a>
                                </div>
                            </div>
                            <button type="button" id="preview-close" class="btn-close" aria-label="Close" style="{{ $banner->is_closable ? '' : 'display: none;' }}"></button>
                        </div>
                    </div>
                </div>
            </div>
...
#banner-preview {
    ...
    @if($banner->background_color || $banner->text_color)
        @if($banner->background_color)
            ...
            border-color: {{ $banner->background_color }};
        @endif
