            ...
            <div class="page-header">
                <div class="page-header-content">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <a href="{{ route('admin.cms.country.create') }}" class="btn btn-primary">
        ...
        <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12 col-12">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h4 class="card-title mb-1">전체 국가</h4>
        ...
        <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12 col-12">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h4 class="card-title mb-1">기본 국가</h4>
                    ...
                    <form method="GET" class="row g-3">
                        <div class="col-md-3">
                            ...
                            <input type="text" name="search" class="form-control"
                                   placeholder="국가명, 코드, 설명" value="{{ request('search') }}">
                        </div>
                        <div class="col-md-2">
                        ...
                        <div class="col-md-2">
                            <label class="form-label">기본 국가</label>
                            <select name="is_default" class="form-select">
                                <option value="all">전체</option>
                                <option value="1" {{ request('is_default') == '1' ? 'selected' : '' }}>기본 국가</option>
                                <option value="0" {{ request('is_default') == '0' ? 'selected' : '' }}>일반 국가</option>
                            </select>
                        </div>
                        <div class="col-md-2">
                            ...
                            <select name="sort_by" class="form-select">
                                ...
                                <option value="name" {{ request('sort_by') == 'name' ? 'selected' : '' }}>국가명</option>
                                <option value="code" {{ request('sort_by') == 'code' ? 'selected' : '' }}>국가 코드</option>
                        ...
                        <div class="col-md-2">
                            ...
                            <a href="{{ route('admin.cms.country.index') }}" class="btn btn-outline-secondary">초기화</a>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="card-title mb-0">국가 목록</h4>
                    <div>
                ...
                <div class="card-body p-0">
                    ...
                    @if($countries->count() > 0)
                            ...
                            <table class="table table-hover mb-0">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th class="sortable" data-sort="name">
                                            국가 정보 <i class="fe fe-chevrons-up-down text-muted ms-1"></i>
                                        </th>
                                        ...
                                        <th width="150" class="sortable" data-sort="is_default">
                                            기본 국가 <i class="fe fe-chevrons-up-down text-muted ms-1"></i>
                                        </th>
                                ...
                                <tbody id="sortable">
                                    ...
                                    <tr data-id="{{ $country->id }}">
                                        <td>
                                            <input type="checkbox" name="country_ids[]" value="{{ $country->id }}" class="form-check-input country-checkbox">
                                        </td>
                                        <td class="text-center">
                                            <span class="fw-bold text-primary">{{ $country->id }}</span>
                                        </td>
                                        <td class="text-center">
                                            @if($country->flag)
                                            ...
                                            <div class="d-flex align-items-center">
                                                <div>
                                                    ...
                                                    @if($country->native_name && $country->native_name !== $country->name)
                                                        <span class="text-muted">({{ $country->native_name }})</span>
                                                    @endif
                                                    <div>
                                                        <span class="badge bg-secondary">{{ strtoupper($country->code) }}</span>
                                                        @if($country->capital)
                                                            <span class="badge bg-light text-dark">{{ $country->capital }}</span>
                                                        @endif
                                                    </div>
                                        ...
                                        <td class="text-center">
                                            <div class="form-check form-switch">
                                                <input class="form-check-input" type="checkbox"
                                                       {{ $country->enable ? 'checked' : '' }}
                                                       onchange="toggleStatus({{ $country->id }}, this.checked)">
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            ...
                                            @else
                                                <button type="button" class="btn btn-outline-warning btn-sm"
                                                        onclick="setDefault({{ $country->id }})" title="기본 국가로 설정">
                                                    <i class="fe fe-star"></i>
                                                </button>
                                            @endif
                                        </td>
                                        <td class="text-center">
                                            <div class="d-flex align-items-center justify-content-center">
                                                <span class="badge bg-info me-2">{{ $country->order }}</span>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-3 px-3 pb-3">
                            <div class="text-muted">
                                총 {{ number_format($countries->total()) }}개 중
                                {{ number_format($countries->firstItem()) }} - {{ number_format($countries->lastItem()) }}개 표시
                            </div>
                            <div>
                    ...
                    @else
                        <div class="text-center py-5 px-3">
                            ...
                            <h5 class="text-muted">국가를 찾을 수 없습니다</h5>
                            <p class="text-muted mb-3">검색 조건을 변경하거나 새로운 국가를 추가해보세요.</p>
                            <a href="{{ route('admin.cms.country.create') }}" class="btn btn-primary">
                                <i class="fe fe-plus me-2"></i>새 국가 추가
                            </a>
                        </div>
...
function updateLanguageOrder() {
    ...
    // 서버에 순서 업데이트 요청
    fetch('/admin/cms/country/update-order', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            showAlert('success', '국가 순서가 업데이트되었습니다.');
            // 순서 컬럼의 값도 업데이트
            updateOrderColumn(orderData);
        } else {
            showAlert('error', '국가 순서 업데이트에 실패했습니다.');
    ...
    .catch(error => {
        ...
        showAlert('error', '국가 순서 업데이트 중 오류가 발생했습니다.');
        location.reload();
    });
}
...
function toggleStatus(id, enabled) {
    fetch(`/admin/cms/country/${id}/toggle`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function setDefault(id) {
    if (confirm('이 국가를 기본 국가로 설정하시겠습니까?')) {
        fetch(`/admin/cms/country/${id}/set-default`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function deleteLanguage(id) {
    if (confirm('정말 삭제하시겠습니까?')) {
        ...
        form.method = 'POST';
        form.action = `/admin/cms/country/${id}`;
...
function bulkAction(action) {
    const selectedIds = Array.from(document.querySelectorAll('.country-checkbox:checked')).map(cb => cb.value);
    ...
    // 일괄 작업 구현
    fetch('/admin/cms/country/bulk-action', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
