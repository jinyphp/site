...
class SeedWelcomeBlocks extends Command
{
    /**
     * The name and signature of the console command.
     */
    protected $signature = 'jiny:seed-welcome-blocks {--fresh : Clear existing blocks first}';

    /**
     * The console command description.
     */
    protected $description = 'Seed welcome blocks from index02-index08 templates into database';
    ...
    public function handle()
    {
        if ($this->option('fresh')) {
            $this->info('Clearing existing welcome blocks...');
            SiteWelcome::truncate();
        }

        $this->info('Seeding welcome blocks...');
        ...
        $this->info('Welcome blocks seeded successfully!');

        // 통계 출력
        $totalBlocks = SiteWelcome::count();
        $totalGroups = SiteWelcome::distinct('group_name')->count();

        $this->info("Total: {$totalGroups} groups with {$totalBlocks} blocks");
    ...
    protected function insertGroupBlocks(string $groupName, string $groupTitle, string $groupDescription, array $blocks): void
    {
        ...
        $this->info("✓ Created {$groupName} ({$groupTitle}) with " . count($blocks) . " blocks");
    }
}